...
int bind_to_overlay(const char *path) {
  ...
  if (written < 0) {
    ...
    written = fputs("failed to build mount path: too few chars written\n", stderr);
    if (written < 0) {
      perror("failed to print error message about mount path building");
    }
    return EXIT_FAILURE;
  }
...
int velen_run(int argc, char* argv[]) {
  ...
  if (argc < 1) {
    err = fputs("command to sandbox not provided\n"
          "usage: velen-run COMMAND [ARGUMENTS]...\n",
          stderr);
    if (err < 0) {
      perror("failed to print usage");
    }
    return EXIT_FAILURE;
  }
